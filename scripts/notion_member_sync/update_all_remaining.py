...
# -*- coding: utf-8 -*-
"""
残りの全てのNotionユーザーIDを一括更新するスクリプト
"""
...
for i, line in enumerate(lines):
    ...
    # 8列以上あれば処理（SlackユーザーID列が欠けている場合もある）
    if len(parts) >= 8:
        ...
        if email_col and email_col in email_to_notion_id:
            ...
            if (current_notion_id.startswith("U") and len(current_notion_id) < 16) or (current_notion_id.startswith("UP") and len(current_notion_id) < 16) or (new_notion_id != current_notion_id and len(new_notion_id) == 36):
